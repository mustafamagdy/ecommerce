...
{
    Task<Order> CreateCashOrder(IEnumerable<OrderItemRequest> items, Customer customer, Guid cashPaymentMethodId,
        CancellationToken cancellationToken);

    Task<Order> CreateCashOrder(IEnumerable<OrderItemRequest> items, Customer customer,
        List<OrderPaymentAmount> payments, CancellationToken cancellationToken);

    Task<Order> CreateOrder(IEnumerable<OrderItemRequest> items, Customer customer, List<OrderPaymentAmount> payments,
        CancellationToken cancellationToken);
}

public class GetServiceCatalogDetailByIdSpec : Specification<ServiceCatalog, ServiceCatalogDto>,
...
public class CreateOrderHelper : ICreateOrderHelper
{
    ...
    public CreateOrderHelper(IRepositoryWithEvents<Order> repository,
    ...
    public Task<Order> CreateCashOrder(IEnumerable<OrderItemRequest> items, Customer customer, Guid cashPaymentMethodId,
        CancellationToken cancellationToken)
        => CreateOrder(items, customer,
            new List<OrderPaymentAmount> { new() { PaymentMethodId = cashPaymentMethodId, Amount = 0 } },
            cashOrder: true, cancellationToken);

    public Task<Order> CreateCashOrder(IEnumerable<OrderItemRequest> items, Customer customer,
        List<OrderPaymentAmount> payments, CancellationToken cancellationToken)
        => CreateOrder(items, customer, payments, cashOrder: true, cancellationToken);

    public Task<Order> CreateOrder(IEnumerable<OrderItemRequest> items, Customer customer,
        List<OrderPaymentAmount> payments, CancellationToken cancellationToken)
        ...
        List<OrderPaymentAmount> payments, bool cashOrder, CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException(_t["Total paid amount {0} doesn't match order net amount {1}",
                totalPaid, orderItemsTotal]);
        }
        ...
        {
            cashRegister.AddOperation(new ActivePaymentOperation(cashRegister, payment.PaymentMethodId, payment.Amount,
                _systemTime.Now, PaymentOperationType.In));
    ...
    private List<OrderItem> GetOrderItems(IEnumerable<OrderItemRequest> items, CancellationToken cancellationToken)
    {
        return items.Select(async item =>
            {
                var serviceItem =
                    await _serviceCatalogRepo.FirstOrDefaultAsync(new GetServiceCatalogDetailByIdSpec(item.ItemId),
                        cancellationToken);
                return new OrderItem(item.ItemId, item.Qty, serviceItem.Price, serviceItem.ProductName,
                    serviceItem.ServiceName, TEMPHelper.VatPercent());
            })
            .Select(a => a.Result)
            .ToList();
    }
