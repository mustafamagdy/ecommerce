...
{
  public CreateCashOrderRequestValidator(IReadRepository<ServiceCatalog> serviceCatalogRepo, IStringLocalizer<IBaseRequest> t)
...
public class CreateCashOrderRequestHandler : IRequestHandler<CreateCashOrderRequest, OrderDto>
{
  ...
  {
    var defaultCustomer = await _customerRepo.FirstOrDefaultAsync(new GetDefaultCashCustomerSpec(), cancellationToken);
