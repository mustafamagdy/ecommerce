...
public static class AraibcPdfExtention
{
  ...
  private static Dictionary<string, string[]> InitializeTableUnicode()
  {
    ...
    xGlyphs.Add("\\u0623",
      new string[] { "\\uFE83", "\\uFE83", "\\uFE84", "\\uFE84", "2" }); // (أ) Alef With Hamza Above
    xGlyphs.Add("\\u0624",
      new string[] { "\\uFE85", "\\uFE85", "\\uFE86", "\\uFE86", "2" }); // (ؤ) Waw With Hamza Above
    xGlyphs.Add("\\u0625",
      new string[] { "\\uFE87", "\\uFE87", "\\uFE88", "\\uFE88", "2" }); // (إ) Alef With Hamza Below
    xGlyphs.Add("\\u0626",
      new string[] { "\\uFE89", "\\uFE8B", "\\uFE8C", "\\uFE8A", "4" }); // (ئ) Yeh With Hamza Above
