<template>
    <div class="column justify-around q-mb-sm col-grow">
                ...
                <div class="q-ma-sm" v-for="record in page.records" :key="record.id" style="min-width: 250px">
                    <q-card class="my-card items-start row text-center" clickable>
                        ...
                        <q-item-section class="column q-ma-sm items-center">
                            <q-item-label class="q-ma-sm">{{ record.name }}</q-item-label>
                            <q-item-label class="q-ma-sm">{{ record.branch }}</q-item-label>
        ...
        <q-dialog v-model="page.showAddOrEdit" persistent> <pointOfSalesAddEdit :showAdd="page.showAdd" /> </q-dialog>
    </div>
</template>
<script setup>
...
import pointOfSalesAddEdit from "./point-of-sales-add-edit.vue";
const page = reactive(
    useCRUDList({
