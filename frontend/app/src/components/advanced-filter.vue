<template>
    <div>
        <q-btn :icon="edit ? 'mdi-close' : 'mdi-playlist-edit'" :class="edit ? 'bg-color-dark' : 'bg-color-info'" @click="edit = !edit" />
        <div class="row items-center q-gutter-sm q-mt-xs" v-for="(filter, index) in data.filters">
            <span v-if="index !== 0">{{ data.logic }}</span>
            <span>
                <q-select :readonly="!edit" v-model="filter.field" :options="fields" option-value="name" option-label="name" />
            </span>
            <span>
            ...
            <q-btn v-if="edit" icon="mdi-delete" class="bg-color-negative" @click="removeFilter(index)" />
            <q-btn v-if="edit" icon="mdi-plus" class="bg-color-positive" @click="addFilterAfter(index)" />
        </div>
        {{ data }}
    </div>
</template>
